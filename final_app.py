...
def summary_stats():
    ...
    monthly_avg = round(total_ships_current_year / months_in_year, 2) if months_in_year > 0 else 0
    ...
    utilization = round((active_ships_today / 80) * 100, 2) if total_ships_current_year > 0 else 0
    return jsonify({
...
def monthly_berth_usage():
    ...
    return jsonify({
        "months": list(grouped.index),  # ["2024-01", "2024-02", ...]
        "berths": list(grouped.columns),  # ["BERTH A", "BERTH B", ...]
        "values": grouped.values.tolist()  # 2D array of counts per berth per month
    })
